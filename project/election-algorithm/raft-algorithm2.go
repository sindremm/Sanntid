...
func (e *ElectionHandler) Heartbeat() {
	heartbeatChannel := make(chan ElectionHandler)

	go bcast.Transmitter(12345, heartbeatChannel)

	for _, elevator := range e.Elevators {
		heartbeatChannel <- ElectionHandler{ID: e.ID, Timestamp: time.Now().UnixNano()}
		go elevator.ReceiveHeartbeat(e.Id)
	}
}

// ReceiveHeartbeat receives heartbeat messages on the given port
func (e *ElectionHandler) ReceiveHeartbeat(port int) {
	heartbeatChannel := make(chan ElectionHandler)
...
func (e *ElectionHandler) CheckHeartbeat(id string, peers_port int, broadcast_port int) {
	peers_update_channel := make(chan peers.PeerUpdate)
	go peers.Receiver(peers_port, peers_update_channel)

	aliveCheck := make(chan elev_structs.AliveMsg)
